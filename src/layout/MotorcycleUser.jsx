...
export default function MotorcyclesUser() {
  ...
  const addToCart = (motorcycle) => {
    ...
    if (motorcycle.status === 'ว่าง') {
      ...
      // Display notification message with green color
      setNotification(<span className="notification-green">{notificationMessage}</span>);
      ...
      // Display notification message with red color
      setNotification(<span className="notification-red">{notificationMessage}</span>);
      // Clear notification after 3 seconds
      setTimeout(() => {
        setNotification('');
      }, 3000);
