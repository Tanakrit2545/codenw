      ...
      <div className="login-border p-5 rounded mt-5">
        ...
        <form className="flex flex-col gap-2 form-container" onSubmit={handleSubmit}>
          ...
          style={{ 
            ...
            fontSize: '24px',
            borderRadius: '10px' 
          }}>{language === 'THAI' ? ' เข้าสู่ระบบ ' : ' LOG IN '}</button>
          </div>
        </form>

        <div className="mt-4 flex justify-center">
  <button className="bg-red-600 text-white px-4 py-2 rounded" onClick={() => alert('Login with Google')}>
    LOGIN WITH GOOGLE
  </button>
</div>
<div className="mt-4 flex justify-center">
  <button className="bg-green-600 text-white px100 py-2 rounded w-40" onClick={() => alert('Login with LINE')}>
    LOGIN WITH LINE
  </button>
</div>
<div className="mt-4 flex justify-center">
  <button className="bg-indigo-600 text-white px-4 py-2 rounded" onClick={() => alert('Login with Facebook')}>
    LOGIN WITH FACEBOOK
  </button>
</div>
